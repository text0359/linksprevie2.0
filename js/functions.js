...
function updateDashboard() {
    ...
    // --- ADIÇÃO: Lógica para o card "Redescubra este Link" ---
    if (items.length > 0) {
        const randomIndex = Math.floor(Math.random() * items.length);
        const randomItem = items[randomIndex];
        const { title, image, description } = randomItem.metadata;
        const shortUrl = randomItem.url.replace(/^(https?:\/\/)?(www\.)?/i, '').split('/')[0];
        
        rediscoverItemContent.innerHTML = `
            <div class="flex items-center gap-4 cursor-pointer" onclick="window.open('${randomItem.url}', '_blank')">
                <img src="${image}" alt="Preview" class="w-24 h-24 object-cover rounded-lg" onerror="this.src='https://placehold.co/96?text=Preview'">
                <div class="flex-1">
                    <h4 class="font-bold text-lg">${title}</h4>
                    <p class="text-sm opacity-80 truncate-lines-2">${description}</p>
                    <p class="text-xs opacity-60 mt-1">${shortUrl}</p>
                </div>
            </div>
        `;
    } else {
        rediscoverItemContent.innerHTML = `<p>${window.translations[currentLanguage]['noItemsToRediscover']}</p>`;
    }
}
...
function applyLanguage(lang) {
  ...
  document.querySelectorAll('[data-i18n]').forEach(element => {
    ...
    if (typeof translation === 'function') {
      // MODIFICAÇÃO: Lida com traduções de função, como o copyright
      if (key === 'copyright') {
        const year = new Date().getFullYear();
        element.innerHTML = translation(year); // innerHTML para renderizar o <span>
      }
    } else if (translation) {
...
function updateFolderSelects() {
  ...
  // ADIÇÃO: Botão da Fila é adicionado no HTML, o event listener virá no main.js
...
async function renderItems(filteredItems = items) {
  ...
  for (const item of filteredItems) {
    ...
    // ADIÇÃO: Indicador visual de prioridade
    const priorityIndicator = item.priority ? '<span class="text-yellow-500 text-lg" title="Alta Prioridade">★</span>' : '';
    ...
    const folderColorHex = FOLDER_COLOR_MAP[folder.color] || FOLDER_COLOR_MAP['bg-gray-500'];
    ...
    const itemHtml = `
      <div class="glass-effect p-3 sm:p-4 rounded-xl hover:shadow-xl transition-all flex flex-col items-center justify-between h-full animated-border" style="--folder-color: ${folderColorHex};">
        ...
        <div class="flex flex-wrap items-center justify-center gap-1 sm:gap-2 mt-4 pt-4 border-t border-white/10 w-full item-actions-bottom">
          ...
          ${starsHtml} 
          ${priorityIndicator} <button class="copy-btn px-2 sm:px-3 py-1 bg-gradient-to-r from-green-600 to-green-700 rounded-lg text-white text-sm hover:from-green-700 hover:to-green-800" data-url="${item.url}" data-i18n="copyBtn">${window.translations[currentLanguage]['copyBtn']}</button>
...
function searchItems() {
  ...
  const priority = searchPrioritySelect.value; // ADIÇÃO
  ...
  const filteredItems = items.filter(item => {
    ...
    // ADIÇÃO: Lógica de filtro por prioridade
    const matchesPriority = priority === 'all' ||
                            (priority === 'high' && item.priority === true) ||
                            (priority === 'normal' && (item.priority === false || item.priority === undefined));

    return matchesQuery && matchesTags && matchesType && matchesFolder && matchesRating && matchesPriority;
  });
...
// --- NOVO: Bloco de Funções para Notificações de Lembrete ---

/**
 * Pede permissão para notificações e agenda um lembrete se permitido.
 */
function handleNotifications() {
    ...
    if (Notification.permission === 'granted') {
        showCustomModal(window.translations[currentLanguage]['notificationsEnabled'], 'OK');
        scheduleReminderNotification();
    } else if (Notification.permission === 'denied') {
        showCustomModal(window.translations[currentLanguage]['notificationsBlocked'], 'OK');
    } else {
        Notification.requestPermission().then(permission => {
            if (permission === 'granted') {
                ...
                showCustomModal(window.translations[currentLanguage]['notificationsDisabled'], 'OK');
            }
        });
    }
}
...
/**
 * Encontra um item antigo e envia uma notificação para o usuário.
 * Lógica simplificada: encontra um item com mais de 30 dias.
 */
function scheduleReminderNotification() {
    const thirtyDaysAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
    const oldItems = items.filter(item => item.timestamp < thirtyDaysAgo);

    if (oldItems.length > 0) {
        const randomOldItem = oldItems[Math.floor(Math.random() * oldItems.length)];
        
        const title = 'Lembrete para Redescobrir!';
        const options = {
            body: `Que tal dar uma olhada neste link que você salvou: "${randomOldItem.metadata.title}"?`,
            icon: './icon.png' // Certifique-se de ter um 'icon.png' na raiz do seu projeto
        };

        const notification = new Notification(title, options);
        notification.onclick = () => {
            window.open(randomOldItem.url, '_blank');
        };
    } else {
        console.log('Nenhum item antigo encontrado para enviar notificação.');
    }
}