...
addItemBtn.addEventListener('click', async () => {
    ...
    if (url) {
        ...
        items.push({
            ...
            timestamp: Date.now(),
            rating: 0,
            priority: false, // MODIFICAÇÃO: Garante que novos itens tenham prioridade normal
            accessHistory: []
        });
        tags.forEach(tag => {
...
searchPrioritySelect.addEventListener('change', searchItems); // NOVO: Listener para filtro de prioridade
...
itemsList.addEventListener('click', (e) => {
    if (e.target.classList.contains('delete-btn')) {
        ...
        if (currentEditingItem) {
            ...
            editItemPriorityCheckbox.checked = currentEditingItem.priority || false; // MODIFICAÇÃO: Define o estado do checkbox
...
saveItemChangesBtn.addEventListener('click', async () => {
    if (currentEditingItem) {
        ...
        const newPriority = editItemPriorityCheckbox.checked; // MODIFICAÇÃO: Captura o novo valor de prioridade
        ...
        currentEditingItem.priority = newPriority; // MODIFICAÇÃO: Salva o novo valor de prioridade
...
tagsList.addEventListener('click', (e) => {
    if (e.target.classList.contains('edit-tag-btn')) {
        ...
        // MODIFICAÇÃO: Usa a função de tradução para a mensagem de confirmação
        showConfirmModal(window.translations[currentLanguage].confirmDeleteTag(tagToDelete), () => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NOVO: Adicionando listeners para as novas funcionalidades ---
